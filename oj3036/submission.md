...
```text
โจทย์ให้รับค่าเลขห้องที่เราอยู่และหาจำนวนกำแพงที่ต้องพังขึ้นไปน้อยที่สุดกี่กำแพง

Input:
รับค่าห้องเข้ามาว่าอยู่ห้องไหน

Output:
ค่ากำแพงที่ต้องพังน้อยที่สุด

Constraints:
ลองดูว่าถ้าใส่ค่าอื่น ๆ ที่ติดลบในโปรแกรมจะเป็นอย่างไร เนื่องจากห้องเป็นจำนวนเต็มบวก
```
...
```text
Step 1: รับ input เลขห้องเข้ามา
Step 2: ตรวจว่าห้องเป็นสามเหลี่ยมคว่ำหรือหงาย
Step 3: แสดง output จำนวนกำแพงที่น้อยที่สุด

ตอนนี้ยังคิดแค่ว่าสามเหลี่ยมคว่ำหงายน่าจะพังกำแพงต่างกัน
```
...
```text
คล้ายวิธีแรกแต่เพิ่มเงื่อไขเข้ามา โดยการหาแพทเทิร์น
   เริ่มด้วยรับค่าเลขห้องเข้ามา จากการคิดหาแพทเทิร์นจากตัวเลขฝั่งขวาสุดเพราะคิดว่ามีนัยสำคัญเนื่องจากทุกตัวเป็นเลขยกกำลังสองทั้งหมด เลขได้ว่าชั้นนั้นเมื่อเอาตัวเองไปกำลังสองถอดรูทแล้วปัดเศษขึ้นจะได้ชั้นที่อยู่ แต่รู้ชั้นไม่พอต้องดูด้วยว่าเป็นสามเหลี่ยมคว่ำหรือหงาย เนื่องจากมีวิธีพังกำแพงต่างกันแม้อยู่ชั้นเดียวกัน
   โดยได้แพทเทิร์นว่าเมื่อเอาเลขห้องไปบวกชั้นที่อยู่จะได้เลขคู่หรือคู่มาโดยเลขคู่จะเป็นหงาย เลขคี่เป็นคว่ำ
เนื่องจากถ้าคว่ำทางที่ไปชั้นอื่นจะทะลุกำแพงไปได้กำแพงเดียวเลย แต่ถ้าหงายต้องทะลุสองกำแพงถึงจะไปชั้นอื่น จึงคิดเป็นแพทเทิร์นตามสูตร 2(n-1) สำหรับหงาย และ 2(n-1)-1 สำหรับคว่ำ(n คือ ชั้นที่อยู่) และสั่ง print output จำนวนห้องที่น้อยที่สุดออกมา โดยได้รับการไกด์จากเพื่อนเพราะก่อนหน้านี้ไม่รู้ว่าคว่ำหวานจะคิดเลขต่างกัน
...
```text
1 เช็คว่าถ้าอยู่ห้องที่ต้องไปเลยจะได้ค่าตามที่ต้องการไหม
```
...
```text
อยากลองว่าถ้าค่าติดลบโปรแกรมจะแสดงผลอย่างไร
```
...
```text
ถ้าใส่เลขข้างนอกที่ไม่ได้อยู่ในรูป
```
...
```text
ไกด์ว่าต้องหาว่าห้องที่ถูก input เข้ามาเป็นสามเหลี่ยมคว่ำหรือหงาย
```

คุณยังทำอะไรด้วยตนเอง

```text
พยายามหาแพทเทิร์นว่ากำแพงที่ต้องทุบมีความเกี่ยวข้องยังไงกับเลขห้องและชั้น
```
