...
```text
 โจทย์ให้ผสมสี จากแม่สี มีสี Red Yellow Blue ซึ่งต้องได้ผลเป็น Red + Yellow = Orange, Red + Blue = Violet, Yellow + Blue = Green  นอกจากแม่สีถ้านำมาผสมกันจะเกิด Error

Input:
 รับค่า string เข้ามาเป็น Red, Yellow, Blue

Output:
ถ้าสี Red + Yellow = Orange
        Red + Blue = Violet
        Yellow + Blue = Green
        นอกจากนี้ให้  print("Error")

Constraints:
ถ้าเป็นสีตรงตามแม่สีให้ส่งสีที่ผสมได้ออกมา แต่ถ้าเป็นสีที่ไม่ใช่แม่สีมาผสมกันหรือแม่สีผสมกับสีที่ไม่ใช่แม่สีให้  Error
...
```text
Step 1: รับ input   สีมาสองสี
Step 2: เช็คว่าสีอะไรผสมกันบ้าง
Step 3: ทำให้แน่ใจว่าสีที่ใส่เข้ามาจะสลับตำปหน่งได้โดยที่สีของ output เป็นสีเดิม
Step 4:ถ้าตรงตามเงื่อนไข print สีที่ผสมได้
Step 5:ถ้าไม่อยู่ในเงื่อนไขให้ print Error
```
...
```text
วิธีสุดท้ายคล้ายกับแผนแรก โดยแผนแรกจะรับค่าสีเข้ามา 2 ค่า จากนั้นนำไปเช็คตามเงื่อนไขการผสมสีและ print สีที่ผสมได้ออกมา โดยถ้าไม่ตรงเงื่อนไขให้ print Error และสิ่งที่เพิ่มเข้ามาคือการเช็คกรณีที่สีที่ใส่มาทั้งสองสีเป็นสีเดียวกันแล้ว print สีนั้นออกมา โดนเงื่อนไขที่เพิ่มมานี้ถามจากเพื่อนเนื่องจากไม่ผ่านการเช็คสีซ้ำกันจนได้ไอเดียมาทำต่อ (ตอนแรกไม่ได้เช็คสีซ้ำ)
```
...
```text
เช็คว่าสีซ้ำจะออกมาเป็นอย่างไรซึ่งลืมเช็คในตอนแรกโค้ดจึงไม่ผ่าน
```
...
```text
เลือกเช็คว่าหาก input อีกอันเป็นสีตามแม่สีอีกอันไม่เป็นจะออกมาเป็นอย่างไร
```
...
```text
เทสว่าไม่ว่าจะขึ้นสีไหนก่อนหลังถ้าเป็นสีที่ผสมได้สีเดียวกันก็ยังได้สีเดิม
```
...
```text
ช่วยถาม  TestCase ที่อาจลืมเช็คไป
```

คุณยังทำอะไรด้วยตนเอง

```text
 เขียนโค้ด TestCase เอง คือ การเช็คสีที่ซ้ำกันต้องได้สีเดิม
```
