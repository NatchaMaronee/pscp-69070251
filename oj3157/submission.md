...
```text
โจทย์ให้รับค่าจำนวน n ครั้งในการรับเครื่องหมาย +,- ซึ่งถ้าเครื่องหมายเป็ฺนบวกให้ +10 ถ้าเครื่องหมายเป็นลบให้ -5
และแสดงผลรวมของเลขที่กรอกไป

Input:
จำนวน N ครั้ง และ +, -

Output:
 แสดงผลรวมของค่าทั้งหมด

Constraints:
เช็ค 0 กันข้อผิดพลาดในการใช้ for loop
```
...
```text
Step 1: รับค่า n เข้ามา หลังจาดนั้นใส่เครื่องหมาย +, - อย่างใดอย่างหนึ่งในแต่ละครั้งจำนวน n ครั้ง
Step 2: ทำการนำ +, .- เก็บไว้ในลิสต์
Step 3: ใช้ count นับ รืยีะที่ตรงกันและนำไปคุณค่านั้น ๆ 
Step 4: จำนวนที่ input เป็น + คูณ 10, จำนวนที่ input เป็น - คูณ -5
Step 5: แสดงผลรวมทั้งหมดของค่าบวกลบ 
```
...
```text
ใช้วิธีเดิม แต่วิธีก่อนหน้านั้นจะใช้ if else แต่คิดว่าจะทำให้โค้ดยาวโดยใช่เหตุจึ่งใช้การ count ค่าในลิสต์แทน ซึ่งได้โค้ดที่ง่ายและสั้นลง
```
...
```text
ทำสอบว่าหาก n = 0 จะได้ผลออกมาเป็น 0 ไหม
```
...
```text
เช็คว่าโค้ดทำงานได้ตรงตามเงื่อนไขใหม่ที่สร้าง (ทุกค่าเป็น -)
```
