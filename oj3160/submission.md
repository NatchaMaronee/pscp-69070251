...
```text
โจทย์ให้รับค่าเป็นช่วงช่วงหนึ่งเข้ามาแล้วให้เช็คทุกค่าในช่วงนั้นว่าค่าใดเป็นจำนวนเฉพาะบ้าง

Input:
ช่วงช่วงหนึ่ง

Output:
ถ้ามีจำนวนเฉพาะให้แสดงค่าจำนวนเฉพาะทั้งหมดแล้วบอกว่ามีกี่ตัว ถ้าไม่มีจำนวนเฉพาะเลยให้ไม่แสดงอะไรแล้วบอกว่าจำนวนเฉพาะมี 0 ตัว

Constraints:
เช็คช่องที่ตัวเริ่มน้อยกว่าตัวจบ
```
...
```text
Step 1: รับค่าช่วงเข้ามาสองค่า (a, b)
Step 2: สร้างลิสต์เพื่อนำไว้เก็บค่าจำนวนเฉพาะ 
Step 3: ใช้ for loop ในการเช็คทุกตัวในช่วง และสร้างตัวแปร count ไว้เก็บค่าทุก ๆ i % k ในอีกลูปแล้วได้เศษ 0
Step 4: ในลูปมีเงื่อไขคือตัวที่จะนำไปเข้าอีกลูปต้องมีค่ามากกว่า 1 ซึ่งในอีกลูปจะเช็คค่าในช่วง 2 ถึงก่อนถึงตัวมันเอง
Step 5: ถ้า count == 0 ให้เพิ่มค่านั้นเข้าลิสต์ซึ่งแสดงว่าค่านั้นเป็นจำนวนเฉพาะ
Step 6: สร้างเงื่อนไขหาก ภายในลิสต์มีค่ามากกว่า 0 ให้แสดงค่าทั้งหมาดในลิสต์พร้อมบอกจำนวน นอกเหนือจากนั้นให้แสดงผลค่าจำนวนเฉพาะเป็น 
```
...
```text
ใช้เหมือนวิธีเดิม แค่คิดได้เป็นระบบขึ้น เพราะเอาทุกตัวภานในช่วงไปเช็คใน range(2, i) เพื่อดูว่ามีตัวก่อนหน้่ที่หารมันลงตัวได้ไหม ซึ่งผลออกมาก็ถูกต้อง
```
...
```text
เช็คกรณีที่ค่าก่อนหน้าน้อยกว่าค่าหลัง
```
...
```text
เชช็คค่าช่วงเป็น 0 0 ว่าโปรแกรมจะทำงานอย่างไร
```
...
```text
เช็คเมื่อมีช่วงลบ ว่าโปรแกรมจะแสดงช่วงลบไหม ซึ่งไม่ถูกต้อง
```
