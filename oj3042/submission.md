...
```text
โจทย์ให้รับค่าเข้ามาแล้วแสดงค่าทุกค่าที่หาร 10 ลงตัวในช่วงค่านี้
Input:
รับค่าจำนวนเต็มบวก

Output:
ค่าทุกค่าในช่วงของ input ที่หาร 10 ลงตัว

Constraints:
ลองเช็ค 0 ดู เผื่อโปรแกรมมีอะไรผิดพลาด
```
...
```text
Step 1: รับค่าจำนวนเต็มเข้ามา
Step 2: เช็คว่าค่านั้นในช่วงของ input หาร 10 ลงตัวไหม
Step 3: ถ้าหารลงตัว print ค่าออกมา โดยเรียงอยู่ในบรรทัดเดียวกันขั้นด้วยช่องว่าง
...
```text
ใช้เหมือนวิธีแรก โดยรับค่า input เข้ามา จากนั้นนำค่าไปหาร 10 เผื่อเอาส่วน จะได้ไม่มีเศษเหลือ จากนั้นนำไปคูณ 10  เพราะเราเอาไปหารสิบแล้วจำนวนตอนนี้ไม่ใช่จำนวนที่หารสิบ เลยต้องคูณกลับ แต่ครั้งนี้กรณีที่จะเกิดเศษจะหายไป

ใช้ while loop เพื่อนำค่าทุก ๆ -10 ออกมาเพราะโจทย์อยากได้ out put จากเลขมากไปน้อย เลยใช้การลบทีละ 10 เอา (start ที่  ค่า input, stop ที่ตำแหน่ง -1 เนื่องจากเป็นการนับถอยหลังจากค่า step จึงตั้งให้หยุดที่ 0 ก่อน -1, step -10 เพื่อนับถอยหลังที่ละ 10) จากนั้น print ใน for loop ให้มาต่อกันโดยไม่เริ่มบรรทัดใหม่และมีช่วงว่างด้วย end = " "
```
...
```text
อยากลองค่า 0 ว่าโปรแกรมจะคิดเลขได้ถูกต้องไหม
```
...
```text
อยากลองว่าถ้าค่าเป็นค่าเศษจะออกมาเป็นอย่างไร
```
...
```text
เช็คว่าถ้าค่าเยอะจะมีอะไรผิดพลาดไหม
```
...
```text
ไกด์วิธีการกำจัดกรณีที่มีเศษง่าย ๆ โดยการหารเอาส่วน
```

คุณยังทำอะไรด้วยตนเอง

```text
เขียนโค้ดการหารเอาส่วนและหาวิธีคิดต่อโดยการคูณ 10 กลับและลดค่าทีละ 10, ส่งผล oj และดันขึ้น git เองทั้งหมด
```
