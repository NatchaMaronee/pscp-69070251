...
```text
โจทย์ให้รับค่า N = จำนวนคน(นั่งเป็นวงกลม) K = ส่งไปให้คนถัดไป(ข้ามกี่คน) T = ตำแหน่งของคนที่เป็นโจร และหาจำนวนคนที่ได้ตรวจเช็คกล่องว่าทั้งหมดมีกี่คน โดยถ้าขโมยได้ตรวจจะขโมยกล่องไปทันที

Input:
N = จำนวนคน(นั่งเป็นวงกลม) K = ส่งไปให้คนถัดไป(ข้ามกี่คน) T = ตำแหน่งของคนที่เป็นโจร

Output:
จำนวนคนที่ได้ตรวจเช็คกล่อง

Constraints:
เช็คกรณีที่ N = 0 ว่าจะผิดเทสเคสไหม
```
...
```text
Step 1: รับค่า N = จำนวนคน(นั่งเป็นวงกลม) K = ส่งไปให้คนถัดไป(ข้ามกี่คน) T = ตำแหน่งของคนที่เป็นโจร
Step 2: ทำการสร้างตัวแปร count (เพื่อนับจจำนวนคนที่ตรวจเช็คกล่อง)
Step 3: ใช้ while loop ถ้าตำแหน่งของคนปัจจุบันไม่ใช่ตำแหน่งของโจร ให้ loop ทำงานต่อไป
Step 4: เซต count = 1 (นับครั้งแรก) เข้าเงื่อนไขถ้า count = 1 แปลว่าคนแรกได้ตรวจ ให้นับ 1 เลย
Step 5: ถ้าตำแหน่งปัจจุบัน = ตำแหน่งโจรให้ break และออกจาก loop ทันที
Step 6: ถ้าไม่ตจรงกับเงื่อนไขด้านบน ให้เข้าสูตร (1 + (count*K)) % N เผื่อหาตำแหน่งปัจจุบัน
Step 7: ถ้าตำแหน่ปัจจุบัน = 1 ให้ count -= 1 และหยุดทันทีแล้วออกจาก loop
Step 8: แสดงผล count 
```
...
```text
ใช้วิธีเดิม แต่เปลี่ยนการคิดไปเรื่อย ๆ (ตรงสูตรคำนวณ) ตอนแรกใช้สูตรที่คิดใหม่เผื่อจะใช้ตัวแปรน้อยลง แต่คิดขึ้นได้ว่าสามารถใช้ count มาคิดแทนตัวแปรใหม่ได้ใน (1 + (count*K)) % N
```
...
```text
เช็คทุกตัวเป็น 0 โปรเgกรมจะแสดงค่าอย่างไร
```
...
```text
เช็คว่าถ้าส่งให้ถัดไป 0 คน โค้ดจะใช้งานได้ไหม
```
...
```text
โค้ดรันไปเรื่อย ๆ (มากจนไม่แสกงผล)
```
...
```text
เช็คกรณีที่คนแรกเป็นโจรเลยว่าจะหยุดทันทีไหม
```
