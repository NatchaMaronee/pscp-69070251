...
```text
โจทย์ให้หาจำนวนที่อยู่ในช่วง (A,B) ที่หารด้วย d และได้เศษ r

Input:
รับช่วง A B เข้ามา รวมถึง d, r

Output:
จำนวนในช่วง (A,B) ที่หารด้วย d และเหลือเศษ r (มีกี่ตัว)

Constraints:
เช็คช่วง 0,0 ดูว่าจะมีปัญหาไหม
```
...
```text
Step 1: รับค่า A B d r เข้ามา
Step 2: สร้างลิสต์ว่างไว้เผื่อนำค่าเข้าในอนาคต
Step 3: ใช้ for loop เพื่อรันเลขทั้งหมดในช้วง (A,B)
Step 4: โดยมีเงื่อนไขในลูปคือ ถ้า i ที่แทนค่าแต่ละช่วงใน (A,B) หาร d แล้วเหลือเศษ r ให้ทำการเพิ่มค่า i เข้าไปในลิสต์
Step 5: ปริ้นท์ output ด้วยการใช้ len() นับว่าในลิสต์มีข้อมูลกี่ตัว
```
...
```text
เหมือนขั้นตอนแรก รับค่า A B d r เข้ามา สร้างลิสต์ว่างไว้เผื่อนำค่าเข้าในอนาคต
ใช้ for loop เพื่อรันเลขทั้งหมดในช้วง (A,B) โดยมีเงื่อนไขในลูปคือ ถ้า i ที่แทนค่าแต่ละช่วงใน (A,B) หาร d แล้วเหลือเศษ r ให้ทำการเพิ่มค่า i เข้าไปในลิสต์ ปริ้นท์ output ด้วยการใช้ len() นับว่าในลิสต์มีข้อมูลกี่ตัว(ข้อมูลในลิสต์ล้วนแล้วตีงตามเงื่อนไขที่ต้องการ)
```
...
```text
เช็คว่าถ้าช่วงเป็น 0 ทั้งคู่จะได้ค่าเป็นอย่างไร
```
...
```text
เช็คว่าถ้าตัวเลขที่นำมาหารเป็น 2 ซึ่งถ้าเราอยากได้เศษ 3 จะไม่มีทางเป็นไปได้ ต้องได้คำตอบ 0
```
...
```text
เช็คว่าถ้าหารด้วย 1 เศษเป็น 0 จะได้ครบไหม
```
