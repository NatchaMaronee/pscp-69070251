...
```text
โจทย์ให้รับจำนวนร้านและเวลาที่จะเช็คมา แล้วทำการแสดงผลว่าแต่ละช่วงเวลามีร้านเปิดกี่ร้าน

Input:
รับค่าจำนวนร้าน และจำนวนช่วงเวลาที่จะเช็ค

Output:
จำนวนร้านที่เปิดของแต่ละช่วงเวลา

Constraints:
พิจารณาเมื่อค่า int เป็น 0
```
...
```text
Step 1: รับค่าจำนวนร้าน จำนวนช่วงเวลาที่จะเช็ค
Step 2: สร้างลิสต์เผื่อนำมาเก็บจำนวนร้านที่เปิดในแต่ละนาที และลิสต์สำหรับเก็บจำนวนร้านที่เปิดในแต่ละช่วงเวลานั้น
Step 3: ใช้ for loop เพื่อวนจนรับค่ามาเต็มและแยกค่าเผื่อนำไปคิดคำนวณต่อ
Step 4: ใช้ for loop ในการเพิ่มช่วงที่ร้านเปิดเข้าไปในลิสต์ที่ทุกนาทีมีค่าเป็น 0 ซึ่งใช้แทนจำนวนร้านที่เปิดในแต่ละนาทีนั้นโดยถ้าบวก 1 ซ้ำเข้าไปเรื่อย ๆ คือร้านเปิดในน่าทีนั้น n ร้าน
Step 5:จากนั้นใช้ for loop กับค่าช่วงเวลาร้านค้าที่จะเช็ค(ซึ่งเราได้ทำการเก็บเป็นลิสต์) เผื่อเพิ่มเข้าลิสต์สำหรับเก็บจำนวนร้านค้า โดยดึงค่าจากลิสต์นาทีในตำแหน่งนาทีที่เรารับอินพุทเข้ามา ซึ่งจะได้เลข 1 ที่บอกกันซ้ำ ๆ ซึ่งแทนจำนวนร้านที่เปิดอยู่ ร ตอนนั้น
Step 6: ปริ้นค่าในลิสต์ออกมาเพื่อแสดงว่าเวลานั้นมีร้านที่เปิดอยู่กี่ร้าน
```
...
```text
เหมือนวิธีเดิมเพราะว่าทำแล้วโค้ดไม่มีปัญหา มีการดึงค่าจาดลิสต์ได้และผ่านเทสเคส โดยที่ใช้มี 2 ลิสต์เผื่อเก็บค่าต่าง ๆ อย่าลิสต์นาทีคือลิสต์ที่เก็บจำนวนร้านที่เปิดซึ่งเช็คใน for loop ที่บวกตำแหน่งนั้น 1 เสมอ ลิสต์ที่สองคือลิสต์ที่ดึงค่าจากตำแหน่งในช่วงเวลาที่ต้องการจาดลิสต์นาที เพื่อนำมาแสดงผล
```
...
```text
เช็คว่าถ้าเวลาที่จะเช็คเป็น 0 ทั้งหมด โค้ดจะมีปัญหาไหม
```
...
```text
เช็คช่วงเวลาที่เปิดแล้วปิดเลยว่าจะมีปัญหาไหม
```
...
```text
เช็คว่าถ้าอยากเช็คช่วงที่ร้านเปิดแล้วร้านปิดพอดีจะแสดงผลอย่างไร
```
...
```text
เพื่อนแนะนำเรื่องที่ใช้ลิสต์มารับค่าวนซ้ำเพื่อหาจำนวนร้านที่เปิดได้
```

คุณยังทำอะไรด้วยตนเอง

```text
สร้างค่าลิสต์เพื่อรับค่าวนซ้ำและลิสต์ที่ดึงค่าในช่วงเวลาที่ต้องการออกมา
```
